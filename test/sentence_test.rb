...
class SentenceTest < Test::Unit::TestCase
  def setup
    tree = Tree::TreeNode.new("1", "R")
    tree << Tree::TreeNode.new("2", "C1") << Tree::TreeNode.new("4", "GC1")
    tree << Tree::TreeNode.new("3", "C2")
    @simple = Sentence.new("kor","eng",tree)
    ...
    nineteen<<twenty_one
    @complicate = 
    @complicate = Sentence.new("kor",'How could you say such a terrible thing to your boss?',one)
  end
  def test_set_height
    nodes = @simple.set_height
  ...
  def test_make_patterns_with_a_real_sample
    patterns = @complicate.make_patterns
    assert(0<patterns["WRB_MD_PRP_VB_PDT_DT_JJ_NN_TO_PRP$_NN_."])

    assert(0<patterns["WRB_MD_PRP_VB_PDT_DT_JJ_NN_TO_NP_."])

    assert(0<patterns["WRB_MD_PRP_VB_PDT_DT_JJ_NN_PP_."])
    assert(0<patterns["WRB_MD_PRP_VB_NP_TO_NP_."])
    assert(0<patterns["WRB_MD_PRP_VB_NP_PP_."])
    
    assert(0<patterns["WRB_MD_PRP_VP_."])
    assert(0<patterns["WRB_MD_NP_VB_NP_PP_."])
    assert(0<patterns["WRB_MD_NP_VP_."])
    
    assert(0<patterns["WRB_SQ_."])
    
    assert(0<patterns["SBARQ"])
    assert(0<patterns["Root"])
  end
end